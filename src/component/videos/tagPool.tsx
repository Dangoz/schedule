    ...
    <div className={TagStyle.wrapper}>
      <div className={TagStyle.tags}>
        {inuse.map((tag, index) => (
          <Tag
            key={tag}
            color={selectedTags.indexOf(tag) !== -1 ? `${colors[index]}-inverse` : `${colors[index]}`}
            onClick={tagToggle}
            className={TagStyle.tag}
            icon={[1, 20, 38].indexOf(index) !== -1 ? <StarFilled /> : <TagOutlined />}
          >
            {tag}
          </Tag>
        ))}

        <Tag color={'default'} className={TagStyle.tag} onClick={moreTags} icon={<SettingTwoTone spin={!noMoreTags} />}>
          {noMoreTags ? 'no more - click to reset' : 'more'}
        </Tag>
      </div>
    </div>
  )
